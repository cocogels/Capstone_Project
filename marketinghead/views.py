...
class CreateAssignQuota(LoginRequiredMixin, CreateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        if self.request.user.is_marketinghead:
            self.users - UserMarketingProfile.objects.filter(is_active=True).order_by('email')
        else:
            self.users = UserMarketingProfile.objects.filter(is_marketinghead=True).order_by('email')
...
class AssignQuotaDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_marketinghead:
            users_mention = list(UserMarketingProfile.objects.filter(is_active=True).values('last_name'))
...
class AssignQuotaUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        def get_context_data(self, **kwargs):
            ...
            user_assign_list = [
                assigned_to.id for assigned_to in context['assign_quota_obj'].assigned_ti.all()
            ]
