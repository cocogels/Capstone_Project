...
from contacts.forms import ICLContactForm
...
class ICLContactListView(TemplateView):
    ...
    def get_queryset(self):
        ...
        if request_post:
            ...
            if request_post.get('tel_number'):
                queryset = queryset.filter(
                    tel_number__icontains=request_post.get('tel_number')
                )
    ...
    def get_context_data(self, **kwargs):
        ...
        if(
            ...
            self.request.POST.get('tel_number') or
            ...
            self.request.POST.get('assignned_to')
        ):
            search=True
...
class ICLContactDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.user.is_marketinghead:
            users_mention - list(User.objects.filter(is_active=True).values('email'))
        elif self.request.user != context['object'].created_by:
            user_mention = [{'last_name': context['object'].created_by.last_name}]
        else:
            users_mention = list(context['object'].assigned_to.all().values('email'))
...
class ICLContactUpdateView(UpdateView):
    ...
    def form_invalid(self, form):
        ...
        return self.render_to_response(
            self.get_context_data(form=fprm)
        )
